...
function format(getctx, items, struct_info, path, output) {
  output = output || []
...
function format_item_into(getctx, decl_node, struct_info, path, output) {
  ...
  if(children[4].type === 'keyword') {
    for(var i = 0, len = names.length; i < len; ++i) {
      ...
      var quant = +deparse(names[i].quantifier, getctx)
