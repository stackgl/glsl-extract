...
function preprocess(_getctx) {
  ...
  function onpreprocessor(token) {
    ...
    inject.pipe(parser()).on('data', function(node) {
      if(!node.parent) {
        result = node
      }
    })
  ...
  function $ignore_until_alternate(token) {
    ...
    if(/^#\s*elif/.test(token.data) && !if_lvl) {
      var inject = tokenizer()
        , result
        , rest
      ...
      return !!if_eval(result) ? $init : $ignore_until_alternate
    }

    if(/^#\s*else/.test(token.data) && !if_lvl) {
      return $init
    }
  ...
  function $collect(token) {
    ...
    collected[collected.length - 1] = token
